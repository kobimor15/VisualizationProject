function aggregate(json_array, country){
    ...
    if(country == 'all'){
        ...
        return [total_confirmed, total_recovered, total_deaths, total_active];
            ...
            return [json_array[json_array.length-1]['confirmed'], json_array[json_array.length-1]['recovered'], json_array[json_array.length-1]['deaths'], json_array[json_array.length-1]['active']]
        }
}
...
function worldRace(country){
    ...
    d3.csv(`../CountryData/cases_per_day_agg_${country}.csv`, function(data) {
        ...
        var colors = ["rgb(000, 000, 255)", "rgb(128, 255, 000)", "rgb(255, 000, 000)", "rgb(255, 128, 000)"]
        var x = d3.scaleBand()
            .range([ 0, width ])
            .domain(domain_array)
            .padding(1);
